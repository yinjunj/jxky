new Vue({
    ...
    methods: {
        ...
        promoIndex: function (e, in1, in2, num, price) {
            ...
            that.motionId=[];
            var type = $(el).find('option:selected').attr('alt');
            var zpType = $(el).find('option:selected').attr('zpType');
            if(type=='mzeng'&&zpType=='2'){
                $(el).siblings().show();
                that.motionZid = that.zpMot
            }else{
                $(el).siblings().hide();
                that.motionZid = [];
            }
            ...
            var total = (num * val[that.pIndex].seal_price).toFixed(2);
            $(el).parent().siblings().find(".sealPce").html('￥' + val[that.pIndex].seal_price);
